...
local function my_on_attach(bufnr)
	...
	vim.keymap.set("n", "x", api.node.navigate.parent_close, opts("Close Parent Node"))
	...
	vim.keymap.set("n", "c", function()
		...
		if not src_stat then
			vim.notify("[nvim-tree] Source does not exist", vim.log.levels.ERROR)
			return
		end
		...
		local function copy_file(src_path, dst_path)
			if uv.fs_stat(dst_path) then
				local choice = vim.fn.confirm(("Overwrite?\n%s"):format(dst_path), "&Yes\n&No", 2)
				if choice ~= 1 then
					return true
				end
			end
		...
		vim.ui.input({ prompt = "Save as: ", default = default_name }, function(name)
			...
			if src_stat.type == "directory" and uv.fs_stat(dst) then
				local choice = vim.fn.confirm(
					("Target exists:\n%s\n\nCopy into " .. "it / overwrite files as needed?"):format(dst),
					"&Yes\n&No",
					2
				)
				if choice ~= 1 then
			...
			if not ok then
				vim.notify(("[nvim-tree] Copy failed: %s"):format(err or "unknown error"), vim.log.levels.ERROR)
				return
			end
